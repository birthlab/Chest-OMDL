...
def main(config):
    ...
    wandb.init(project="CT", config={"epochs": config.epochs, "batch_size": config.batch_size, "learning_rate": config.lr})
    sys.path.append(config.work_dir + '/')
    log_dir = os.path.join(config.work_dir, 'log')
    checkpoint_dir = os.path.join(config.work_dir, 'checkpoints')
    # resume_model = os.path.join(checkpoint_dir, 'latest.pth')
    resume_model='path/to/best.pth'
    resume_model='/FM_data/bxg/CT_Report/CT_Report8_16abn_2decoder/results/segmamba__Saturday_04_January_2025_15h_02m_55s/checkpoints/best.pth'
